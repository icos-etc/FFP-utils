...
| FFP.domain	        | Domain size [m] (xmin xmax ymin ymax)                                              |
| dx	                | Cell size of domain [m] (default is dx = dy = 1 m)                                 |
...
| save.plot.FFP.mtx  | If TRUE, saves a jpeg plot of FFP (both matrix and isopleths)                      |
| return.isopleth.70 | If TRUE, includes 70% isopleth in the netCDF file                                  |
| save.log           | If TRUE, saves log messages to a file                                              |
| save.ncplot	       | If TRUE, saves a sample plot of the nc file produced                               |
...
Tower coordinates, site ID and PID can be omitted if the dataframe already contains the them (e.g. the CSV file saved by the CPtoFFPinput function), otherwise they must be specified (e.g. list returned by the CPtoFFPinput function). The function relies on the FFP model function (Kljun et al., 2015) which should be placed in the R working directory, preferably inside the subfolder ‘local_functions’ (default). Alternatively, is also possible to specify a different FFP function path. Please be aware that the FFP model run is time-consuming.

Gaps are filled up to a defined threshold (4 by default; max.gap argument). FFP function-related parameters, such as the spatial extent (1000 m by default; FFP.domain argument), resolution (1 m by default; dx argument) and the FFP density thresholds for isopleths calculation (c(50, 70, 80, 90) by default; FFP.R argument) can be specified. In addition, pixels with a minimal contribution to the total density (threshold of 90%; drop.trivial.srcs argument) are removed by default. 
...
The dimensions are related to the spatial (x, y) and temporal resolution (time), to the coordinates of the EC tower (EC_Coords) and to the 70th isopleth (instance: total number of polygons; nodes: total number of vertices; nchar: maximum number of characters of each entry in the isopleth attribute table). 
...
| geometry_container, UTM_Coordinate_System | They define the associations between the variables linked to the geometry (the 70% isopleth) and the correct Coordinate Reference System (CRS, in WGS84/UTM) for each variable that must be spatially located. The right CRS is automatically detected starting from the WGS84 coordinates of the EC tower |
| nodes_count (instance), nodes_x (nodes), nodes_y (nodes) | Respectively, count of the total number of vertices for each 70% isopleth, X and Y coordinates of each 70% isopleth vertex |
...
doFFP(FFP.input.df = Site_1[['FFP_input_parameters']],
      ...
      max.gap = 4, FFP.domain = 1000, dx = 1, FFP.R = c(50,70,80,90),
      drop.trivial.srcs = TRUE, save.FFP.mtx.as="nc",          
      save.plot.FFP.mtx = FALSE, return.isopleth.70=TRUE, save.log=TRUE, save.ncplot=TRUE)
```
```
library("purrr")
...
```
```
# Processing a single daily file for site using a csv table generated using the CPtoFFPinput function
